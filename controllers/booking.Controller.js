...
const createGuestPatient = async ({ patient_name, patient_phone }) => {
  ...
  try {
    const [user] = await User.create(
      [{ email: guestEmail, password: hashedPassword, user_type: "patient" }],
      { session },
    );
...
const getBookingTarget = async ({ doctor_id, staff_id }) => {
  if (doctor_id) {
    const doctor = await Doctor.findById(doctor_id)
      ...
      .lean();
    if (!doctor || !doctor.is_verified || !doctor.user_id?.is_active)
  ...
  if (
    ...
    staff.clinic_id?.status !== "approved"
  ) {
    throw new AppError("Doctor is not available", 404);
  }
...
const assertSlotAvailable = async ({
  ...
  const overlap = await Booking.findOne({
    ...
    status: "confirmed",
    $and: [
      { booking_from: { $lt: booking_to } },
      { booking_to: { $gt: booking_from } },
    ],
  }).lean();
...
const assertPatientAvailability = async ({
  ...
  const dayConflict = await Booking.findOne({
    patient_user_id,
    ...dayMatchField,
    booking_date,
    status: "confirmed",
  ...
  if (dayConflict)
    throw new AppError(
      "Patient already has a booking with this doctor today",
      409,
    );
};
...
const createBookingRecord = async ({
  ...
  if ((!doctor_id && !staff_id) || (doctor_id && staff_id))
    throw new AppError(
      "Booking must target either a doctor or a staff member",
      400,
    );
...
const findPatientByName = async ({
  ...
  const patientName =
    typeof patient_name === "string" ? patient_name.trim() : "";
  const patientPhone =
  ...
  return {
    patient_id: activePatients[0]._id,
    patient_user_id: activePatients[0].user_id._id,
    full_name: activePatients[0].full_name,
    phone: activePatients[0].phone,
  };
};
...
const getProviderTargetForUser = async ({ user, staff_id }) => {
  ...
  if (!staff_id && staffHasSchedule)
    return { doctor_id: null, staff_id: staff._id };
  if (!staff_id)
    throw new AppError(
      "staff_id is required for non-doctor staff bookings",
      400,
    );
  ...
  if (
    !staffDoctor ||
    !staffDoctor.user_id?.is_active ||
    staffDoctor.clinic_id?.status !== "approved"
  ) {
...
exports.getMyBookings = catchAsync(async (req, res, next) => {
  ...
  const result = await Promise.all(
    bookings.map(async (b) => {
      const patient = await Patient.findOne({ user_id: b.patient_user_id })
  ...
  res
    .status(200)
    .json({ status: "success", results: result.length, bookings: result });
});
  ...
  const result = await Promise.all(
    bookings.map(async (b) => {
      ...
      const staff = b.staff_id
        ? staffList.find((s) => String(s._id) === String(b.staff_id))
        : null;
...
exports.getAvailableSlots = catchAsync(async (req, res, next) => {
  ...
  let target;
  if (doctor_id) {
    target = await Doctor.findById(doctor_id)
      .select("work_days work_from work_to is_verified")
      .lean();
    if (!target || !target.is_verified)
      return next(new AppError("Doctor is not available", 404));
  } else {
    target = await Staff.findById(staff_id)
      .select("work_days work_from work_to is_verified")
      .lean();
    if (
      !target ||
...
exports.cancelClinicBooking = catchAsync(async (req, res, next) => {
  ...
  const booking = await Booking.findById(booking_id)
    .populate("staff_id", "clinic_id")
    .lean();
  if (!booking) return next(new AppError("Booking not found", 404));
  if (String(booking.staff_id?.clinic_id) !== String(clinic_id))
    return next(new AppError("Access denied", 403));
  if (booking.status === "cancelled")
    return next(new AppError("Booking is already cancelled", 400));
...
exports.updateBookingStatus = catchAsync(async (req, res, next) => {
  ...
  if (user_type === "patient") {
    ...
    authorized =
      String(booking.patient_user_id) === String(user_id) &&
      status === "cancelled";
  } else if (user_type === "doctor") {
  ...
  if (!authorized) {
    return next(
      new AppError("Access denied or unauthorized status transition", 403),
    );
  }
